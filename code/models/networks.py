...
def weights_init_normal(m, std=0.02):
    ...
    if classname.find('Conv') != -1:
        init.normal_(m.weight_g.data, 0.0, std)
...
def define_G(opt):
    ...
    task = opt['task']
    if which_model == 'TIM':
        netG = arch.TIM(in_nc=opt_net['in_nc'], code_nc=opt_net['code_nc'], out_nc=opt_net['out_nc'],
                        ...
                        num_levels=opt['levels'], feat_scales=opt_net['feat_scales'], map_nc=opt_net['map_nc'],
                        latent_nc=opt_net['latent_nc'], task=task)
    else:
